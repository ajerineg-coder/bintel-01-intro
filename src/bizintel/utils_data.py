...
WHY: These functions are used in every module of the BI course.
They handle the common tasks of loading, inspecting, and checking
data quality so we do not repeat this logic in every script.
"""
...
def load_data(filepath: Path, name: str) -> pd.DataFrame:
    ...
    in almost every BI pipeline. We log the shape so we can
...
def inspect_basic(df: pd.DataFrame, name: str) -> None:
    ...
    WHY: Before doing any analysis, we need to understand
    what columns exist, what types they are, and what the
...
def check_quality(df: pd.DataFrame, name: str) -> None:
    ...
    WHY: Real-world data is messy. Before loading data into
    a pipeline, we need to know what problems exist so we
...
def summarize_numeric(df: pd.DataFrame, name: str) -> None:
    ...
    WHY: Summary statistics (min, max, mean) give a quick
    sense of the range and distribution of numeric data.
